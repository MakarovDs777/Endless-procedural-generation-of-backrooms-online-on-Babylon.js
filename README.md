...
Как вообще работает процедурная генерация? На примере игры Starbound самая такая обычная процедурная генерация, вот на примере дома, вот нам нужно сделать процедурную генерацию дома в игре как это делается? Берется делается 20 типов крыш, 20 типов дверей, 20 там типов окон, т.д. И делается всевозможная вариация всевозможных комбинаций элементов этого дома, и получается огромное число домов так работает каноническая процедурная генерация.
